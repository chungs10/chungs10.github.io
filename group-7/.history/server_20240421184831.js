...
app.put("/profile/:uid", (req, res) => {
  ...
  async function updateProfile(){
    try {
      ...
      const uri = process.env.MONGODB;
      const client = new MongoClient(uri);
      await client.connect(); // Fix for Problem 1: Added parentheses to await client.connect()
      const database = client.db(Museo).collection("users"); // Fix for Problem 2: Removed quotation marks around Museo
